...
namespace Espeon.Bot.Commands
{
    ...
    public class Roles : EspeonModuleBase
    {
            ...
            SocketRole role)
        {
            ...
            {
                await Task.WhenAll(Context.User.AddRoleAsync(role, new RequestOptions
                {
                    AuditLogReason = "Self assigning role"
                }), SendOkAsync(1));
                return;
            }
            ...
            SocketRole role)
        {
            ...
            await Task.WhenAll(Context.User.RemoveRoleAsync(role, new RequestOptions
            {
                AuditLogReason = "Self assigning role"
            }), SendOkAsync(2));
        }
