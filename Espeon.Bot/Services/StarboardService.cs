    ...
    public class StarboardService : BaseService<InitialiseArgs>, IStarboardService
    {
        ...
        public StarboardService(IServiceProvider services) : base(services)
        {
            _client.ReactionAdded += (cache, channel, reaction)
                => _events.RegisterEvent(() => ReactionAddedAsync(cache, channel, reaction));

            _client.ReactionRemoved += (cache, channel, reaction)
                => _events.RegisterEvent(() => ReactionRemovedAsync(cache, channel, reaction));
        }
        ...
        private async Task ReactionRemovedAsync(Cacheable<IUserMessage, ulong> msg, ISocketMessageChannel channel, SocketReaction reaction)
        {
            ...
            try
            {
                ...
                var foundMessage = guild.StarredMessages
                    .Find(x => x.Id == message.Id || x.StarboardMessageId == message.Id);
