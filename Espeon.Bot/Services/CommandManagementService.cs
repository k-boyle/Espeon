...
namespace Espeon.Bot.Services
{
    public class CommandManagementService : BaseService<InitialiseArgs>, ICommandManagementService
    {
        ...
        async Task<bool> ICommandManagementService.AddAliasAsync(EspeonContext context, Module module, string command, string alias)
        {
            var commands = _commands.GetAllCommands();
        ...
        async Task<bool> ICommandManagementService.RemoveAliasAsync(EspeonContext context, Module module, string alias)
        {
        ...
        async Task<bool> ICommandManagementService.RemoveAliasAsync(EspeonContext context, Module module, string command, string alias)
        {
            var foundModule = await context.CommandStore.Modules.Include(x => x.Commands)
                .FirstOrDefaultAsync(x => x.Name == module.Name);
        ...
        private static Action<ModuleBuilder> OnBuilding(ModuleInfo info)
        {
            return builder =>
            {
                ...
                {
                    var foundCommand = info.Commands.Find(x => x.Name == command.Name);
