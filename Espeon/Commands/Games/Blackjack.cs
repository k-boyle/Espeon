...
namespace Espeon.Commands.Games
{
    public class Blackjack : IGame, IReactionCallback
    {
        ...
        public async Task StartAsync()
        {
            ...
            _ = Task.Delay(Timeout.GetValueOrDefault()).ContinueWith(async _ =>
            {
                ...
                if (_inGame)
                {
                    ...
                    await Candy.UpdateCandiesAsync(Context.User.Id, false, -_bet);
                    _inGame = false;
                }
        ...
        private Embed BuildEmbed()
        {
            ...
            builder.AddField("Espeon", $"My cards are {string.Join(", ", _dealerCards.Select(x => $"[{x.card} of {x.suit}]"))}\n" +
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    if (dealerTotal > 21)
                    {
                        ...
                        await Candy.UpdateCandiesAsync(Context.User.Id, false, (int)(Payout * _bet));
                    }
