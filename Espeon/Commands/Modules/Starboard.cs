	...
	*/
	...
	public class Starboard : EspeonModuleBase {
		...
		public async Task ViewRandomStarAsync() {
			...
			string jump = Utilities.BuildJumpUrl(Context.Guild.Id, randomStar.ChannelId, randomStar.Id);

			Embed starMessage = Utilities.BuildStarMessage(user, randomStar.Content, jump, randomStar.ImageUrl);

			string m = string.Concat($"{Utilities.Star}", $"**{randomStar.ReactionUsers.Count}** - ",
