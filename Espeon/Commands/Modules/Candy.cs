	...
	*/
	...
	public class Candy : EspeonModuleBase {
		...
		public async Task ViewCandiesAsync([Remainder] IGuildUser user = null) {
			...
			int amount = await CandyService.GetCandiesAsync(Context, user);
		...
		public async Task ClaimCandiesAsync() {
			(bool isSuccess, int amount, TimeSpan cooldown) = await CandyService.TryClaimCandiesAsync(Context, User);
		...
		public async Task ViewHouseAsync() {
			int amount = await CandyService.GetCandiesAsync(Context, Client.CurrentUser);
		...
		public Task TreatUserAsync(int amount, [Remainder] IGuildUser user = null) {
			...
			return Task.WhenAll(CandyService.UpdateCandiesAsync(Context, user, amount),
		...
		public Task GiftCandiesAsync(IGuildUser user,
			...
			return user.Id == User.Id
				? SendNotOkAsync(0)
				: Task.WhenAll(CandyService.TransferCandiesAsync(Context, User, user, amount), SendOkAsync(1));
		}
