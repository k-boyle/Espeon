...
namespace Espeon.Commands.Modules
{
    ...
    public class CandyCommands : EspeonBase
    {
        ...
        {
            var count = 1;
            var users = DatabaseService.GrabAllData<UserObject>("users")
                .OrderByDescending(x => x.RareCandies)
                .Select(x => $"{count++} - {(Context.Guild.GetUser(x.Id)?.GetDisplayName() ?? Context.Client.GetUser(x.Id)?.Username ?? "-User not found-")} : {x.RareCandies}").ToArray();
