	...
	public class CommandStore : DbContext {
		...
		protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder) {
			optionsBuilder.UseNpgsql(
				"Host=127.0.0.1;Port=5432;Database=CommandStore;Username=postgres;Password=casino");
		}
#endif
		...
		protected override void OnModelCreating(ModelBuilder modelBuilder) {
			modelBuilder.Entity<ModuleInfo>(module => {
				...
				module.HasMany(x => x.Commands).WithOne(y => y.Module).HasForeignKey(z => z.ModuleName);
			});

			modelBuilder.Entity<CommandInfo>(command => {
				command.HasKey(x => new {
					x.Name,
					x.ModuleName
				});
			});
